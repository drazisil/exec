...
export class ExportEntry {
    ...
    private _forwarder: string | null;

    constructor(ordinal: number, rva: number, name: string | null, forwarder: string | null) {
...
export class ExportTable {
    ...
    private _ordinalBase: number;
    private _timeDateStamp: number;
    private _entries: ExportEntry[];

    constructor(data: Buffer, fileImage: Buffer, sections: SectionHeader[], exportDirRva: number, exportDirSize: number) {
        this._entries = [];
        this._dllName = '';
        this._ordinalBase = 0;
    ...
    toString() {
        if (this._entries.length === 0) return 'Export Table: empty';
