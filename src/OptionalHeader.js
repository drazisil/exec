...
export class OptionalHeader {
    constructor(data) {
        ...
        for (let i = 0; i < this._numberOfRvaAndSizes; i++) {
            ...
            this._dataDirectories.push(new DataDirectory(data.subarray(offset, offset + DataDirectory.sizeOf), i));
        }
    }
    get isPE32Plus() {
    ...
    toString() {
        ...
        lines.push(`ImageBase:                    ${hex(this._imageBase)}`, `SectionAlignment:             ${hex(this._sectionAlignment)}`, `FileAlignment:                ${hex(this._fileAlignment)}`, `OperatingSystemVersion:       ${this._majorOperatingSystemVersion}.${this._minorOperatingSystemVersion}`, `ImageVersion:                 ${this._majorImageVersion}.${this._minorImageVersion}`, `SubsystemVersion:             ${this._majorSubsystemVersion}.${this._minorSubsystemVersion}`, `Win32VersionValue:            ${this._win32VersionValue}`, `SizeOfImage:                  ${hex(this._sizeOfImage)}`, `SizeOfHeaders:                ${hex(this._sizeOfHeaders)}`, `CheckSum:                     ${hex(this._checkSum)}`, `Subsystem:                    ${hex(this._subsystem, 4)}`, `DllCharacteristics:           ${hex(this._dllCharacteristics, 4)}`, `SizeOfStackReserve:           ${hex(this._sizeOfStackReserve)}`, `SizeOfStackCommit:            ${hex(this._sizeOfStackCommit)}`, `SizeOfHeapReserve:            ${hex(this._sizeOfHeapReserve)}`, `SizeOfHeapCommit:             ${hex(this._sizeOfHeapCommit)}`, `LoaderFlags:                  ${hex(this._loaderFlags)}`, `NumberOfRvaAndSizes:          ${this._numberOfRvaAndSizes}`, ``, `Data Directories:`, ...this._dataDirectories.map((dd, i) => `  [${i.toString().padStart(2)}] ${dd}`));
        return lines.join('\n');
    }
}
