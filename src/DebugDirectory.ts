...
export class DebugDirectoryEntry {
    ...
    private _sizeOfData: number;
    private _addressOfRawData: number;
    private _pointerToRawData: number;
    private _pdbPath: string | null;
    private _pdbGuid: string | null;
    private _pdbAge: number | null;

    static get sizeOf() { return 28; }

    constructor(data: Buffer, fileImage: Buffer) {
    ...
    private formatGuid(bytes: Buffer): string {
    ...
    toString() {
        ...
        if (this._pdbPath) {
            str += `\n  PDB: ${this._pdbPath}`;
            if (this._pdbGuid) str += `\n  GUID: ${this._pdbGuid}`;
            if (this._pdbAge !== null) str += `  Age: ${this._pdbAge}`;
        }
        return str;
    }
}
...
export class DebugDirectory {
    private _entries: DebugDirectoryEntry[];
    ...
    toString() {
        if (this._entries.length === 0) return 'Debug Directory: empty';
