...
export class EXEFile {
    constructor(filePath, dllSearchPaths = []) {
        this._filePath = "";
        ...
        this._fileImage = (readFileSync(this._filePath));
        ...
        for (let i = 0; i < this._coffFileHEader.numberOfSections; i++) {
            ...
            this._sectionHeaders.push(new SectionHeader(this._fileImage.subarray(offset, offset + SectionHeader.sizeOf)));
        }
    ...
    parseDataDirectories() {
        ...
        const dir = (index) => dirs[index]?.data.length > 0 ? dirs[index] : null;
        // [0] Export Table
