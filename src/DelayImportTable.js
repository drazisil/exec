import { rvaToOffset } from "./helpers.ts";
import { ImportEntry } from "./ImportTable.ts";
export class DelayImportDescriptor {
    constructor(dllName, attributes, moduleHandle, iat, int, boundIAT, unloadIAT, timeDateStamp, entries) {
...
export class DelayImportTable {
    constructor(data, fileImage, sections, isPE32Plus) {
        this._descriptors = [];
        if (data.length === 0)
            return;
    ...
    parseThunks(fileImage, sections, intRva, firstThunkRva, isPE32Plus) {
        const entries = [];
    ...
    readHintName(fileImage, sections, rva, iatRva, iatFileOffset, iatValue, entries) {
